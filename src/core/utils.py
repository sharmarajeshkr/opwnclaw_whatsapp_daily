...
def trigger_qr_script(raw: str) -> None:
    ...
    script_content = (
        ...
        "        print('Connected! Sending welcome message and allowing crypto sync...')\n"
        "        await asyncio.sleep(5)\n"
        "        try:\n"
        "            welcome = 'Welcome to Interview prepare, this will help u with sugggested   innterview topics and daily updated content'\n"
        "            await c.send_message(welcome)\n"
        "        except Exception as e:\n"
        "            print(f'Warning: could not send welcome message: {e}')\n"
        "        # Start the persistent bot daemon for this user\n"
        "        import subprocess, sys\n"
        f"        subprocess.Popen([sys.executable, 'main.py', '--phone', '{raw}'], cwd=os.getcwd(), start_new_session=True)\n"
        "        print('Bot daemon started. Pairing script exiting cleanly.')\n"
        "        await asyncio.sleep(5)\n"
        "    finally:\n"
        "        try:\n"
        "            os.remove(__file__)\n"
